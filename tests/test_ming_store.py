...
class MingStoreTest(unittest.TestCase):
    def setUp(self):
        pass

    def tearDown(self):
        conn = create_datastore('mim://').conn
        conn.drop_database('metahosting_tests')
        print 'Tearing down MingTests. Remaining dbs:'
        for a in conn.database_names():
            print 'DB %s' % a
    ...
    def get_store(self):
        ...
        return MingStore(config=config)
